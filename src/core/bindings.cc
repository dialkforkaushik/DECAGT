...
PYBIND11_MODULE(decagt, m) {
    py::class_<SimplicialComplex>(m, "SimplicialComplex")
        ...
        .def("compute_boundary_matrices", &SimplicialComplex::compute_boundary_matrices)
    ...
    py::class_<GeometryComplex, SimplicialComplex>(m, "GeometryComplex")
        ...
        .def("compute_dual_volumes", &GeometryComplex::compute_dual_volumes)
        .def("compute_dual_volume_k", &GeometryComplex::compute_dual_volume_k)
        .def("compute_primal_volumes", &GeometryComplex::compute_primal_volumes)
    ...
    py::class_<FiniteElementExteriorCalculus, GeometryComplex>(m, "FiniteElementExteriorCalculus")
		.def(py::init<>())
        .def(py::init<SimplicialComplex>())
		.def("compute_hodge_stars", &FiniteElementExteriorCalculus::compute_hodge_stars)
        .def("compute_hodge_star_k", &FiniteElementExteriorCalculus::compute_hodge_star_k)
